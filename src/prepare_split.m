...
allInputs=[]; allTargets=[]; provenance=strings(0,1);
for fileIndex=1:numel(files)
    ...
    for snrIndex=1:numel(snrLevels)
        ...
        provenance=[provenance;repmat(files(fileIndex),size(inputs,4),1)]; %#ok<AGROW>
    end
end
dataset=struct('inputs',allInputs,'targets',allTargets,'recording',provenance);
end
